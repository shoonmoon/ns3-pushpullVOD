 ...
 * \ingroup PushPull
 *
 * \brief Reads and stores information about a PushPull swarm read from a ".torrent" file.
 *
 * This class holds all information necessary for a client to participate in a PushPull swarm.
 * The information about a PushPull swarm is immutable and it is sufficient that only one
 ...
 *
 * See the <a href="http://wiki.theory.org/PushPullSpecification#Metainfo_File_Structure" target="_blank">PushPull Protocol Specification</a>
...
private:
  ...
  uint8_t                    m_privateMediaData;             // Is this a private torrent?
  ...
  std::string                m_dataPath;                   // The path to the data associated with the MediaData relative to the exec path of ns3
...
public:
  ...
  /**
   * \brief Read the contents of a ".torrent" file into the MediaData class.
   *
   ...
   */
  bool ReadMediaDataFile (std::string path);
...
public:
  ...
  /**
   * \brief Set the location of the data associated with the MediaData.
   *
   * @param dataPath the path to the data associated with this MediaData relative to the ns3 execution path.
   */
  void SetDataPath (std::string dataPath);

  /**
   * @returns the path to the data associated with this MediaData, relative to the ns3 execution path.
   */
   ...
   *
   * This function can be used by a centralized, simulated tracker (such as the PushPullTracker application) to set the correct announce URL
  ...
  /**
   * @returns the comment included within the ".torrent" file. This information may include further input, e.g., for the PushPullVideoClient class.
   */
  ...
  uint8_t IsPrivateMediaData () const;
