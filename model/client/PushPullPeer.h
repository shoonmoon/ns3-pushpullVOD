 ...
 * \ingroup PushPull
 *
 * \brief Implements the PushPull Peer Wire Protocol connection endpoint.
 *
 * This class represents the TCP-based communication endpoint for a connection with another peer held by a PushPull client.
 ...
 *
 * The Peer class exposes a number of methods to send PushPull Peer Wire Protocol messages over an established communication channel.
 ...
 *
 * All default PushPull Peer Wire Protocol messages as defined in the <a href="http://wiki.theory.org/PushPullSpecification#Messages" target="_blank">Bittorrent Protocol Specification v1.0</a> are supported.
 * Additionally, you can send messages adhering to the <a href="http://www.pushpull.org/beps/bep_0010.html" target="_blank">PushPull Extension Protocol (BEP 10)</a>.
 *
 * Note: The sending and analysis of a PushPull Extension Protocol handshake message are currently not implemented. However, you may still use the SendExtendedMessage member function to send messages other
...
private:
  // Main attributes
  Ptr<PushPullClient>           m_myClient;              // The client that this peer is associated with
  ...
  PushPullLengthHeader          m_lengthHeader;          // Stores the announced size of the next PushPull PeerWire Protocol message
  PushPullTypeHeader            m_typeHeader;            // Stores the announced type of the next PushPull PeerWire Protocol message
...
// Constructors etc.
public:
  Peer (Ptr<PushPullClient> myClient);
  virtual ~Peer ();
...
public:
   ...
   *
   * The client, upon a call to this method, tries to connect to the PushPull client at the given address. It automatically creates the
   ...
   *
   * @param silent whether to inform other parts of the PushPull client implementation about the closed connection.
   */
  ...
  /**
   * \brief Send an PushPull Extension Protocol message to the peer.
   *
   * This method creates and inserts an <a href="http://www.pushpull.org/beps/bep_0010.html" target="_blank">PushPull Extension Protocol (BEP 10)</a>
   ...
   *
   * Note: The PushPull Extension Protocol defines a handshake message that shall be sent before initiating communication using this protocol
...
public:
   ...
   *
   * The possible states as defined in the file PushPullDefines.h for each piece are:\n
