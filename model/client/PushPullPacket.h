 ...
 * \ingroup PushPull
 *
 * \brief The PushPull handshake message.
 *
 * This class implements the PushPull handshake message as a derivative of the Header class.
 */
class PushPullHandshakeMessage : public Header
{
// Fields
private:
...
// Getters, Setters
   ...
   *
   * The standard setting is "PushPull protocol".
   *
   ...
   *
   * This setting has no algorithmic meaning in standard PushPull but may be used in PushPull-based protocols for further
 ...
 */
class PushPullLengthHeader : public Header
{
// Fields
private:
...
public:
  PushPullLengthHeader ();
  PushPullLengthHeader (uint32_t len);
  ~PushPullLengthHeader ();
  static TypeId GetTypeId (void);
 ...
 */
class PushPullTypeHeader : public Header
{
// Fields etc.
public:
  enum PushPullMessageType
  {
    KEEP_ALIVE = -1,
    CHOKE = 0,
    UNCHOKE = 1,
    ...
    CANCEL = 8,
    PORT = 9,
    EXTENDED = 20,  // PushPull Extension Protocol according to BEP-10,
	HANDSHAKE = 128
  };

private:
  PushPullMessageType m_type;        // The type of the message that is following in the stream
...
public:
  ...
  /**
   * \brief Construct a PushPullMessageType header.
   *
   * @param type the desired type.
   */
  PushPullTypeHeader (PushPullMessageType type);
  ~PushPullTypeHeader ();
  static TypeId GetTypeId (void);
...
public:
   ...
   */
  PushPullMessageType GetMessageType () const
  {
    return m_type;
  }
   ...
   */
  void SetMessageType (PushPullMessageType type)
  {
    m_type = type;
  }
...
public:
   ...
   */
  PushPullHaveMessage (uint32_t pieceIndex);
  ~PushPullHaveMessage ();
  static TypeId GetTypeId (void);
...
public:
   ...
   */
  PushPullBitfieldMessage (uint32_t bitFieldSize);
  ~PushPullBitfieldMessage ();
  static TypeId GetTypeId (void);
...
public:
   ...
   */
  PushPullRequestMessage (uint32_t pieceIndex, uint32_t blockOffset, uint32_t blockLength);
  ~PushPullRequestMessage ();
  static TypeId GetTypeId (void);
 ...
 * \ingroup PushPull
 *
 * \brief A PushPull PIECE message.
 *
 * This class implements the Peer Wire BITFIELD message as a derivative of the Header class.
 */
class PushPullPieceMessage : public Header
{
// Fields
private:
...
public:
   ...
   */
  PushPullPieceMessage (uint32_t index, uint32_t begin);
  ~PushPullPieceMessage ();
  static TypeId GetTypeId (void);
 ...
 * \ingroup PushPull
 *
 * \brief A PushPull CANCEL message.
 *
 * This class implements the Peer Wire CANCEL message as a derivative of the Header class.
 *
 * This message is the "reverse" of the REQUEST message handled by the PushPullPushPullRequestMessage class.
 */
class PushPullCancelMessage : public Header // This message is the "reverse" of a REQUEST message
{
// Fields
private:
...
public:
   ...
   */
  PushPullCancelMessage (uint32_t pieceIndex, uint32_t blockOffset, uint32_t blockLength);
  ~PushPullCancelMessage ();
  static TypeId GetTypeId (void);
 ...
 */
class PushPullPortMessage : public Header
{
// Fields
private:
...
public:
   ...
   */
  PushPullPortMessage (uint16_t listenPort);
  ~PushPullPortMessage ();
  static TypeId GetTypeId (void);
 ...
 * \ingroup PushPull
 *
 * \brief A PushPull Extension Protocol message.
 *
 * This class implements the <a href="http://www.pushpull.org/beps/bep_0010.html" target="_blank">PushPull Extension Protocol (BEP 10)</a>PushPull Extension Protocol</a> message.
 *
 ...
 */
class PushPullExtensionMessage : public Header
{
// Fields
private:
...
public:
   ...
   */
  PushPullExtensionMessage (uint32_t contentLength);
   ...
   */
  PushPullExtensionMessage (uint8_t messageId, const std::string& content);
  ~PushPullExtensionMessage ();
  static TypeId GetTypeId (void);
