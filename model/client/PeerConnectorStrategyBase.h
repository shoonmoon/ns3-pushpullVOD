...
#include "PushPullHttpClient.h"
#include "PushPullUdpClient.h"
 ...
 * \ingroup PushPull
 *
 * \brief Implements a HTTP-tracker-based peer discovery mechanism for PushPull swarms.
 *
 * This class implements the "traditional" HTTP-tracker based method to discover and connect to peers in a PushPull swarm.
 * It uses a simplified HTTP client implementation provided by the PushPullHttpClient class
 * to communicate with both internal (i.e., ns-3 based) and external PushPull trackers
 * using the standardized HTTP-based PushPull Tracker protocol.
 *
 * This class implements several methods commonly used in PushPull peer discovery mechanisms and applies them to the tracker-based approach.
...
protected:
  ...
  PushPullHttpClient m_httpCC;                       // The HTTP client used to contact the tracker
...
// Constructors etc.
public:
  PeerConnectorStrategyBase (Ptr<PushPullClient> myClient);
...
public:
   ...
   *
   * Note: This setting may be overridden by the discovery mechanism. This is consistent with the a href="http://wiki.theory.org/PushPullSpecification" target="_blank">theory.org specification</a>.
   *
   ...
   *
   * @returns the number of PushPull clients to which the client holds active connections plus the number of peers which the client is currently establishing a connection with.
   */
...
public:
  /**
   * \brief Connect the PushPull client with the cloud.
   *
   ...
   *
   * The client will listen on the specified port for incoming connections and assume that these connections are initiated by PushPull clients.
   *
   * The default port for standard PushPull is 6881-6889. Note that using a port not in this range may result in certain clients being unable to establish a
...
protected:
   ...
   *
   * This method checks if the connection with a peer was successfully established, e.g., that PushPull's Peer Wire handshakes
...
protected:
   ...
   *  <a href="http://wiki.theory.org/PushPullSpecification#Tracker_Request_Parameters" target="_blank">PushPull Protocol Specification v1.0</a>. The passed values are automatically hex-escaped.
   * @param closeCurrentConnection if set to true, a currently-ongoing connection with the discovery mechanism is closed and a new one initiated.
*  Can be used to prioritize events.
   */
