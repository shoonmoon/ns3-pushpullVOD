...
#ifndef PUSHPULLCLIENT_H_
#define PUSHPULLCLIENT_H_
 ...
 * \ingroup PushPull
 *
 * \brief A PushPull client application for ns-3.
 *
 * This class provides a PushPull client application model for ns-3. The client model ships with an implementation of the PushPull protocol that adheres to Bram Cohen's official <a href="http://www.pushpull.org/beps/bep_0003.html" target="_blank">PushPull Protocol Specification</a>
 * and provides client logic according to the descriptions found on the <a href="http://wiki.theory.org/PushPullSpecification" target="_blank">PushPull Protocol Specification on wiki.theory.org</a>.
 * This class is a centralized point of information storage and -exchange for one instance of the PushPull client model.
 *
 * The central idea of the client model is the ability for researchers to exchange or enhance the client logic in order to test protocols based on the PushPull paradigm with low effort.
 * For this reason, the client logic is intended to be subdivided into logical units referred to as "strategies". Classes implementing strategies can be notified of PushPull's Peer Wire protocol messages and exchange information with other classes of the same client instance by triggering and listening to events handled by this PushPullClient class.
 * Communication with other members of a PushPull swarm is provided by methods of the Peer class.
 *
 * For a general overview of the architecture of the system, see the WNS3 2012 workshop paper "Building a modular PushPull model for ns-3".\n
 ...
 * <i>PushPull parameters</i>: Control the central parameters of a client within a PushPull network, such as number of peers and request sizes.
 *
 * <b>3. Internal data</b>\n
 ...
 */
class PushPullClient : public Application
{
// Fields
private:
  // The main attributes of the PushPullClient
...
private:
  ...
  std::list<Callback<void, Ptr<PushPullClient> > >       m_cloudConnectionEstablishedEventListeners;
  std::list<Callback<void, Ptr<PushPullClient> > >       m_cloudConnectionSuspendedEventListeners;

  // Listeners for tracker-related events
  std::list<Callback<void> >                                         m_trackerResponseReceivedListerners;

  // Listeners for PushPull Peer Wire Protocol (PWP) messages, sorted by ID
  ...
  std::list<Callback<void, Ptr<PushPullClient> > >                 m_applicationInitializedEventListeners;
...
public:
   ...
   *
   * A call to this method initializes the PushPull client application by attaching the appropriate strategies to the client,
...
public:
  ...
  /**
   * @returns a string identifying the protocol this instance of the PushPull client implements.
   */
  ...
  /**
   * \brief Set the protocol the instance of the PushPull client shall use.
   *
   * The string identifies the protocol to be attached to the PushPullClient class by the ProtocolFactory.
   *
   * Set this to "default" to choose the default PushPull implementation with sequential part selection, or to "rarest-first"
   * to implement standard PushPull behavior with a rarest-first part selection strategy. Identifiers for further
   ...
   *
   * Note: By default, this returns the IP address of the first Interface of the Node the PushPullClient Application is installed on.
   * You may need to override this method to support other scenarios.
   *
   * @returns the IP address assigned to the first interface of the Node that the PushPullClient class is installed on.
   */
  virtual Ipv4Address GetIp () const;
  
  /**
   * \brief Set the port the PushPull client should listen on for incoming connections.
   *
   * Use this method to make the PushPull client open a specific port for incoming connections if you have installed multiple clients on one specific or interface.
   *
   * @param the port the local server for incoming connection should listen on. Default is 6881, PushPull's default range is 6881-6889.
   */
  void SetPort (uint16_t port);

  /**
   * \brief Get the port the PushPull client is listening on for incoming connections.
   *
   * @returns the port the local server for incoming connection attempts of PushPull clients is listening on.
   */
   ...
   *
   * Note: By default, this method returns the downlink speed of the first Interface of the Node the PushPullClient Application is installed on.
   * You may need to override this method to support other scenarios.
   *
   ...
   *
   * Note: By default, this method returns the uplink speed of the first Interface of the Node the PushPullClient Application is installed on.
   * You may need to override this method to support other scenarios.
   *
   ...
   * PushPullClient class. Each call to this method issues a subsequent StrategyOptionsChangedEvent to which strategies
   ...
   *
   * Note: For easiness of implementation, options once set are retained during the lifetime of the PushPullClient class.
...
public:
  ...
  // PushPull Peer Wire Protocol (PWP) messages, sorted by ID
   ...
   *
   * See PushPullClient::PeerConnectionEstablishedEvent() for the related event.
   */
  ...
  /**
   * \brief This event is triggered upon the reception of a message of the <a href="http://www.pushpull.org/beps/bep_0010.html" target="_blank">PushPull Extension Protocol (BEP 10)</a>.
   *
...
public:
  ...
  void RegisterCallbackApplicationInitializedEvent (Callback<void, Ptr<PushPullClient > > eventCallback);
  void UnregisterCallbackApplicationInitializedEvent (Callback <void, Ptr<PushPullClient > > eventCallback);
  ...
  void RegisterCallbackCloudConnectionEstablishedEvent (Callback<void, Ptr<PushPullClient> > eventCallback);
  void UnregisterCallbackCloudConnectionEstablishedEvent (Callback<void, Ptr<PushPullClient> > eventCallback);
  void RegisterCallbackCloudConnectionSuspendedEvent (Callback<void, Ptr<PushPullClient> > eventCallback);
  void UnregisterCallbackCloudConnectionSuspendedEvent (Callback<void, Ptr<PushPullClient> > eventCallback);
...
public:
   ...
   * inserting the PushPull client into the cloud.
   */
  void JoinCloud ();
