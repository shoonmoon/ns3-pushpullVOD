...
NS_LOG_COMPONENT_DEFINE ("PushPullHttpClient");
NS_OBJECT_ENSURE_REGISTERED (PushPullHttpClient);

PushPullHttpClient::PushPullHttpClient ()
{
...
bool PushPullHttpClient::GetRequestActive () const
{
  return m_requestActive;
}

bool PushPullHttpClient::HasBadHeader () const
{
  return m_badHeader;
}

bool PushPullHttpClient::HttpGetRequest (Ptr<Node> node, TypeId typeidvar, Ipv4Address addr, uint16_t port, std::string path, Callback<void, Ptr<Socket> > ReplyHandler)
{
  if (m_requestActive)
    {
      NS_LOG_INFO ("PushPullHttpClient: Pending request. Aborting new request.");
      return false;
    }
...
void PushPullHttpClient::CloseAndReInit ()
{
  if (m_requestActive)
    {
      m_socket->Close ();
...
std::string PushPullHttpClient::HttpReceiveReply (Ptr<Socket> socket, bool waitUntilFinished, bool dataOnly, size_t& dataStart, bool& finished)
{
  ...
  if (m_dataStarted)
    {
      ...
      if (m_bytesToRead == -1)
        {
          ...
          if (checkPos == std::string::npos)
            {
              NS_LOG_DEBUG ("PushPullHttpClient: Bad Header: No Content-Length field found.");
          ...
          else
            {
              ...
              if (checkPos2 == std::string::npos || checkPos2 >= m_dataStart)
                {
                  NS_LOG_DEBUG ("PushPullHttpClient: Bad Header: No return after Content-Length field found.");
              ...
              else
                {
                  ...
                  if (!converter)
                    {
                      NS_LOG_DEBUG ("PushPullHttpClient: Content-Length could not be converted.");
      ...
      NS_LOG_DEBUG ("PushPullHttpClientConnector: Should read " << m_bytesToRead << " starting at " << m_dataStart << "; have: " << m_buffer.size () << ".");
      ...
      if (finished|| !waitUntilFinished)
        {
          if (finished)
            {
              NS_LOG_DEBUG ("PushPullHttpClientConnector: Transfer finished. Bytes to read: " << m_bytesToRead << "; buffer size: " << m_buffer.size () << ".");
            }
