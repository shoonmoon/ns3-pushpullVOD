...
#ifndef PUSHPULL_TRACKER_H
#define PUSHPULL_TRACKER_H
...
typedef std::map<std::string, std::string>      PPDict;
typedef std::map<std::string, PPDict>           PPDoubleDict;

/**
 * \ingroup PullPush
 *
 * \brief Implements a simple, HTTP-based PullPush tracker.
 *
 * This class implements a HTTP-based tracker for PullPush swarms.
 * It uses a simplified HTTP server implementation provided by the PullPushHttpServer class
 * to communicate with both internal (i.e., ns-3 based) and external PullPush clients
 * using the standardized HTTP-based PullPush Tracker protocol.
 *
 ...
 */
class PullPushTracker : public Application
{
...
protected:
   ...
   * @cond HIDDEN
   */
  struct PullPushTrackerCloudInfo
  {
    PPDoubleDict             m_clients;          // Peer id as index; also stores seeders!
    PPDoubleDict             m_seeders;          // To store seeders only (to more easily retrieve them)
    PPDoubleDict             m_leftSeeders;      // Those seeders who have left the cloud (for logging purposes)
    PPDict                   m_info;             // Various information on the cloud
...
private:
  // Main attributes
  PullPushHttpServer       HttpCS;             // The HTTP server this tracker uses
...
protected:
  /// @cond HIDDEN
  std::map<std::string, PullPushTrackerCloudInfo> m_cloudInfo;     // The clouds that the tracker serves; info hash as index
  /// @endcond HIDDEN
...
private:
  PullPushTracker (const PullPushTracker&);
  PullPushTracker& operator = (const PullPushTracker);
...
public:
   ...
   * to this setting; it is solely used to more accurately mimic PullPush traffic on networks.
   *
...
public:
   ...
   *
   * The returned Torrent object can be directly used as an input to the PullPushClient class.
   *
   * @param path the path (relative to the current execution directory) to the ".torrent" file to be loaded.
   *
   * @returns a pointer to a simulation-global Torrent class instance that contains all data needed by the PullPushClient class to work within a specific
   ...
   *
   * Note that, by the PullPush protocol, clients have to re-announce themselves with event "started" in order to be added to the swarm by the tracker.
   *
...
public:
   ...
   *
   * This method creates a tracker response to a PullPush client.
   * The message is bencoded according to <a href="http://www.bittorrent.org/beps/bep_0003.html" target="_blank">Bram Cohen's official PullPush specification</a>.
   * The default implementation returns up to <a href="http://wiki.theory.org/PullPushSpecification#Tracker_Request_Parameters" target="_blank">numwant</a> randomly-selected swarm members
   * as the client list.
   *
   ...
   */
  std::string GenerateResponseForPeer (const PPDict& clientInfo) const;
   ...
   *
   * The HTTP GET request in the supplied string is converted into STL a dictionary of type std::map<std::string, std::string> (PPDict),
   ...
   * @returns the request in a STL dictionary of type std::map<std::string, std::string> (PPDict).
   */
  static PPDict ExtractInfoFromClientMessage (std::string path);
